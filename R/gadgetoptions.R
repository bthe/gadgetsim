...
##' @name gadget.options
##' @param type type of model (either 'simple2stock' or 'spawning')
...
gadget.options <- function(type=c('simple2stock','spawning')){
  ...
  if(type=='simple1stock'){
    ...
    opt$fleets$surv$suitability <- subset(opt$fleets$surv$suitability, opt$fleets$surv$suitability$stock=='imm')
    opt$fleets$comm$suitability <- subset(opt$fleets$comm$suitability, opt$fleets$surv$suitability$stock=='imm')
  }
...
gadget.skeleton <- function(time,area,stocks,fleets=NULL){
  ...
  stocks <- 
    plyr::llply(stocks,function(x){
      print(x$name)
      Growth <- methods::new('gadget-growth',
        ...
        if(is.null(x$renewal.data)){
          x$renewal.data <- 
            plyr::mutate(subset(getTimeSteps(time), 
                                getTimeSteps(time)$step == x$renewal.step),
      ...
      if(x$doesspawn==1){
        spawndata <- methods::new('gadget-spawning',
      ...
      stock <- methods::new('gadget-stock',
                            stockname = x$name,
  ...
  if(!is.null(fleets)){
                            ...
                            if(x$type %in% c('linearfleet','effortfleet')){
                              fleetdat <- 
                                plyr::mutate(area.temp[c('year','step','area')],
                                             fleet=x$name,
                                             Fy=x$Fy)
                              fleetdat <- subset(fleetdat,fleetdat$step==x$catchstep)
                            } else {
                              fleetdat <- x$amount
                            }
    ...
    fleets <- methods::new('gadget-fleet')
  }
  
  model <- methods::new('gadget-main',
                        model.name='gadget-model',
                        time = time,
                        area = area,
                        print = list(),
                        stocks = stocks,
                        tags = methods::new('gadget-tagging'),
                        otherfood = list(),
                        fleets = fleets,
                        likelihood = list()
  )
